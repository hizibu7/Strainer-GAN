# 상위 10% 제거해서 fake image에 concate
#%matplotlib inline
import argparse
import os
import random
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        if epoch >= 3:
            with torch.no_grad():
                ...
                filtered_real = real_cpu[mask]
                filtered_fake = real_cpu[~mask]  # 필터링된 "가짜 같은" 실제 데이터
                b_size_real = filtered_real.size(0)
                b_size_fake = filtered_fake.size(0)
        else:
            filtered_real = real_cpu
            b_size_real = b_size
            b_size_fake = 0

        # 진짜 데이터로 학습
        label_real = torch.full((b_size_real,), real_label, dtype=torch.float, device=device)
        output = netD(filtered_real).view(-1)
        errD_real = criterion(output, label_real)
        errD_real.backward()
        D_x = output.mean().item()

        # 가짜 데이터로 학습
        noise = torch.randn(b_size - b_size_fake, nz, 1, 1, device=device)
        fake = netG(noise)
        if epoch >= 3:
            fake = torch.cat([fake, filtered_fake], dim=0)  # 생성된 가짜 데이터와 필터링된 실제 데이터를 결합
        ...
        ###########################
        netG.zero_grad()
        label_g = torch.full((fake.size(0),), real_label, dtype=torch.float, device=device)  # 생성자의 손실값을 구하기 위해 진짜 라벨을 이용합니다
