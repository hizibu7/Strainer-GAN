...
def estimate_ratio_dbscan(dataset, eps=20, min_samples=3):
    ...
    # -1 레이블은 노이즈 포인트를 나타냅니다
    clean_ratio = np.sum(labels != -1) / len(labels)
    
    return clean_ratio



def detect_outliers(dataset, feature_extractor, clean_ratio):
    ...
    # clean_ratio를 기반으로 threshold 설정
    threshold = torch.quantile(max_z_scores, clean_ratio)
...
# DBSCAN을 사용하여 clean 데이터 비율 추정
clean_ratio = estimate_ratio_dbscan(combined_dataset)
print(f"Estimated clean data ratio: {clean_ratio:.4f}")

# 추정된 clean_ratio를 사용하여 이상치 제거
is_inlier = detect_outliers(combined_dataset, feature_extractor, clean_ratio)
clean_dataset = torch.utils.data.Subset(combined_dataset, torch.where(is_inlier)[0])
