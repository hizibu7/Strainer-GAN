#clean 분포와 noisy 분포가 만나는 지점의 loss보다 작은 데이터들만 clean 샘플로 간주
...
for epoch in range(num_epochs):
    ...
    clean_dataset, _ = divide_dataset(losses, combined_dataset)
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        iters += 1
         ...
         # 에폭이 끝날 때마다 전체 데이터셋으로 다시 설정
        dataloader = torch.utils.data.DataLoader(combined_dataset, batch_size=batch_size, shuffle=True, num_workers=workers)