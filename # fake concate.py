...
# potential_fake 인덱스 중 10%만 랜덤하게 선택
num_potential_fake = len(potential_fake_indices)
num_selected_fake = int(num_potential_fake * 0.1)  # 10% 선택
...
potential_fake_dataloader = torch.utils.data.DataLoader(potential_fake_dataset, batch_size=batch_size,
                                                        shuffle=True, num_workers=workers)

print(f"Total potential fake images: {num_potential_fake}")
print(f"Selected potential fake images: {num_selected_fake}")



# 학습 과정
print("Starting Training Loop...")
first_refined_dataset = combined_dataset  # 초기 정제된 데이터셋 저장
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        if epoch >= 3:  # 3 epoch부터 potential fake 데이터 사용
            # potential fake 이미지 가져오기
            try:
                potential_fake_data = next(iter(potential_fake_dataloader))
            except StopIteration:
                potential_fake_dataloader = iter(torch.utils.data.DataLoader(potential_fake_dataset, 
                                                                             batch_size=b_size, 
                                                                             shuffle=True, 
                                                                             num_workers=workers, 
                                                                             drop_last=True))
                potential_fake_data = next(potential_fake_dataloader)
            
            potential_fake_images = potential_fake_data[0].to(device)

            # 생성된 가짜 이미지와 potential fake 이미지 합치기
            combined_fake = torch.cat([fake, potential_fake_images], dim=0)
        else:
            combined_fake = fake
...
# Fake 이미지 준비
fake_dataloader = torch.utils.data.DataLoader(fake_tensors, batch_size=64, shuffle=True, num_workers=workers)
