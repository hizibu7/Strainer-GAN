...
original_dataset = clean_dataset  # 초기 정제된 데이터셋 저장

for epoch in range(num_epochs):
    if epoch >= 3:  # 3 epoch부터 loss 기반 이상치 제거 적용
        clean_dataset, threshold = refine_dataset_by_loss(original_dataset, netD, device, user_loss_ratio)
        if len(clean_dataset) > 0:
            dataloader = torch.utils.data.DataLoader(clean_dataset, batch_size=batch_size,
                                                     shuffle=True, num_workers=workers)
            print(f"Epoch {epoch}: Refined dataset size: {len(clean_dataset)}, Threshold: {threshold:.4f}")
        else:
            print(f"Warning: Clean dataset is empty. Using original dataset for epoch {epoch}")
            dataloader = torch.utils.data.DataLoader(original_dataset, batch_size=batch_size,
                                                     shuffle=True, num_workers=workers)
