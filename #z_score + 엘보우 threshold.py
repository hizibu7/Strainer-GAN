...
def visualize_z_scores(z_scores, threshold, bin_centers, hist):
    ...
    plt.xlabel('Z-Score')
    plt.ylabel('Density')
    plt.title('Distribution of Z-Scores with New Threshold')
    plt.legend()
    plt.show()

def detect_outliers(dataset, feature_extractor):
...
is_inlier = detect_outliers(combined_dataset, feature_extractor)
