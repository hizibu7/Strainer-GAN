...
def train_autoencoder(autoencoder, dataloader, num_epochs=5):
...
def detect_outliers_autoencoder(autoencoder, dataset, device, threshold=2.0):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            output = autoencoder(img)
            error = F.mse_loss(output, img, reduction='none').view(img.size(0), -1).mean(dim=1)
            reconstruction_errors.append(error.cpu())
    
    reconstruction_errors = torch.cat(reconstruction_errors, dim=0)
    threshold_value = reconstruction_errors.mean() + threshold * reconstruction_errors.std()
    is_inlier = reconstruction_errors < threshold_value
    return is_inlier
...
# 원본 데이터셋을 저장
original_combined_dataset = combined_dataset
...
for epoch in range(num_epochs):
    ...
    # 3 epoch 이후부터 매 epoch 마다 데이터셋 리셋 및 이상치 제거
    if epoch >= 3:
        # 전체 데이터셋으로 리셋
        combined_dataset = original_combined_dataset
        
        # 이상치 탐지 및 제거
        is_inlier = detect_outliers_autoencoder(autoencoder, combined_dataset, device)
        combined_dataset = torch.utils.data.Subset(combined_dataset, torch.where(is_inlier)[0])
        
        # 새로운 DataLoader 생성
        dataloader = torch.utils.data.DataLoader(combined_dataset, batch_size=batch_size,
                                                 shuffle=True, num_workers=workers)
        
        print(f"Epoch {epoch}: Removed {(~is_inlier).sum()} outliers. {len(combined_dataset)} samples remaining.")
