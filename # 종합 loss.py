...
# CIFAR-10에서 20,000장 무작위 선택
cifar_sample_size = 20000
cifar_indices = random.sample(range(len(cifar10_dataset)), cifar_sample_size)
cifar_subset = torch.utils.data.Subset(cifar10_dataset, cifar_indices)

print(f"CelebA dataset size: {human_dataset_size}")
print(f"CIFAR-10 sampled size: {cifar_sample_size}")
...
# 데이터셋 결합 및 섞기
combined_dataset = ShuffledCombinedDataset(human_dataset, cifar_subset)
...
def get_percentile_threshold(losses, percentile=75):
    return np.percentile(losses, percentile)

def get_iqr_threshold(losses):
    Q1 = np.percentile(losses, 25)
    Q3 = np.percentile(losses, 75)
    IQR = Q3 - Q1
    return Q3 + 1.5 * IQR

def get_ensemble_threshold(losses):
    gmm_threshold = get_gmm_threshold(losses)
    percentile_threshold = get_percentile_threshold(losses)
    iqr_threshold = get_iqr_threshold(losses)
    
    return np.median([gmm_threshold, percentile_threshold, iqr_threshold])

def divide_dataset(losses, dataset):
    threshold = get_ensemble_threshold(losses)
    
    clean_idx = losses < threshold
    noisy_idx = ~clean_idx
...
for epoch in range(num_epochs):
    ...
    if epoch >= 3:
        losses = evaluate_dataset(netD, combined_dataset, device)
        clean_dataset, noisy_dataset = divide_dataset(losses, combined_dataset)
        
        # 점진적으로 clean 데이터의 비율을 조정
        num_clean = int(len(clean_dataset) * clean_ratio)
        clean_dataset = torch.utils.data.Subset(clean_dataset, range(num_clean))
        
        dataloader = torch.utils.data.DataLoader(clean_dataset, batch_size=batch_size,
                                                 shuffle=True, num_workers=workers)
        
        print(f"Epoch {epoch}: Using {len(clean_dataset)} clean samples out of {len(combined_dataset)}")
        print(f"Removed {len(noisy_dataset)} noisy samples")
    else:
    ...
    # 에폭이 끝날 때마다 전체 데이터셋으로 다시 설정 (3 epoch 이후에만 필요)
    if epoch >= 3:
        dataloader = torch.utils.data.DataLoader(combined_dataset, batch_size=batch_size,
                                                 shuffle=True, num_workers=workers)